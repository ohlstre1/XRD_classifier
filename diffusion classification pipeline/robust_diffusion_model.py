...
class RobustResidualBlock(nn.Module):
    """
    Residual block with LayerNorm instead of GroupNorm for better compatibility.
    """
    ...
    def forward(self, x, time_emb):
        ...
        # First convolution
        h = self.conv1(x)
        h = h.permute(0, 2, 1)  # [B, L, C] for LayerNorm
        h = self.norm1(h)
        h = h.permute(0, 2, 1)  # [B, C, L] back for conv
        h = self.act1(h)
...
class RobustAttention(nn.Module):
    """
    Self-attention block with LayerNorm.
    """
...
class RobustDownBlock(nn.Module):
    """
    Downsampling block using robust components.
    """
    def __init__(self, in_channels, out_channels, time_channels, num_res_blocks=2, attention=False):
        ...
        for i in range(num_res_blocks):
            ...
            self.res_blocks.append(RobustResidualBlock(channels, out_channels, time_channels))

        self.attentions = nn.ModuleList([
            RobustAttention(out_channels) if attention else nn.Identity()
...
class RobustUpBlock(nn.Module):
    """
    Upsampling block using robust components.
    """
    def __init__(self, in_channels, out_channels, time_channels, num_res_blocks=2, attention=False):
        super().__init__()
...
class RobustMiddleBlock(nn.Module):
    """
    Middle block using robust components.
    """
    def __init__(self, channels, time_channels, attention=True):
        super().__init__()

        self.res_block1 = RobustResidualBlock(channels, channels, time_channels)
        self.attention = RobustAttention(channels) if attention else nn.Identity()
        self.res_block2 = RobustResidualBlock(channels, channels, time_channels)
...
# -------------------------------
# Robust Diffusion Model
# -------------------------------
class RobustDiffusionDenoiser(nn.Module):
    """
    A robust 1D U-Net denoiser model using LayerNorm for better compatibility.
    """
    def __init__(self, in_channels=1, hidden_channels=64, time_embedding_dim=128,
        ...
        for i in range(num_levels):
            ...
            self.downs.append(RobustDownBlock(
                in_channels=in_ch,
                out_channels=out_ch,
        ...
        # Output layers using LayerNorm
        self.norm_out = nn.LayerNorm(hidden_channels)
...
# -------------------------------
# Clean Diffusion Process (same as before)
# -------------------------------
...
# -------------------------------
def train_robust_diffusion(model, diffusion, dataloader, val_dataloader=None,
                          num_epochs=50, lr=1e-4, weight_decay=1e-5, device='cpu', save_path='./models'):
    """
    Training function for robust diffusion model.
    """
        ...
        if val_dataloader is not None:
            ...
            if avg_val_loss < best_val_loss:
                ...
                torch.save({
                    ...
                    'loss': best_val_loss,
                    'history': history
                }, f"{save_path}/robust_diffusion_model_best.pth")
...
def main():
    """
    Example usage of the robust diffusion model.
    """
    ...
    dummy_data = torch.randn(1000, 1, 512)
    dummy_conditions = torch.randn(1000, 1)
    ...
    history, trained_model = train_robust_diffusion(
        ...
        lr=1e-4,
        device=device,
        save_path='./models/robust_diffusion'
    )

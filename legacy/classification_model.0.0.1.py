...
class XRDDataHandler:
    ...
    def load_data(self, dataset_path, sample_limit=None):
        ...
        # We'll ignore any existing labels since we'll use custom labels (0-999)
    ...
    def prepare_datasets(self, synth_xrd, real_xrd, labels, train_ratio=0.99, val_ratio=0.01):
    ...
    def create_dataloaders(self, train_dataset, val_dataset, test_dataset, batch_size=32):
...
class XRDModelTrainer:
    ...
    def train(self, train_loader, val_loader, num_epochs=50, learning_rate=0.001, weight_decay=1e-5):
...
def plot_confusion_matrix(y_true, y_pred, num_classes=None):
    ...
    # Also log confusion matrix as a table
...
def main():
    ...
    wandb.init(
        ...
        config={
            ...
            "dataset": "xrd_dataset_labeled_dtw_window",
            "epochs": 50,
            "batch_size": 4,
            "learning_rate": 0.001,
            "weight_decay": 1e-5,
    ...
    sample_limit = 1000  # Set to None to use all data
    batch_size = wandb.config.batch_size
    num_epochs = wandb.config.epochs
    num_classes = 1000  # 1000 different classes (0-999)
    ...
    # Create labels from 0 to 999 as specified
    labels = torch.arange(0, 1000, 1)[:len(synth_xrd)]
    
    # Prepare datasets - training on synth_xrd and testing on real_xrd
    train_dataset, val_dataset, test_dataset = data_handler.prepare_datasets(synth_xrd, real_xrd, labels)
    ...
    model = XRDClassifier(input_length=input_length, num_classes=num_classes).to(device)
    ...
    #plot_confusion_matrix(targets, predictions, num_classes)

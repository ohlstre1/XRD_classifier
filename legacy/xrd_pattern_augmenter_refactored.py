#!/usr/bin/env python3
"""
XRD Pattern Augmenter: Generate Realistic Experimental-like Patterns (Refactored)
===============================================================================

This script uses the refactored XRD diffusion model as an augmenter to transform
...
- Uses modular refactored diffusion model

Usage:
    python xrd_pattern_augmenter_refactored.py --input_file  --output_dir ./augmented --samples_per_pattern 5
"""
...
# Add the refactored diffusion model to the path
sys.path.append('./diffusion_refactored')
...
class XRDPatternAugmenter:
    """
    High-performance XRD pattern augmenter using refactored diffusion model.
    """

    def __init__(self, model_path, device='auto', verbose=True):
        """
        ...
        if self.verbose:
            print(f"✅ XRD Pattern Augmenter initialized successfully in model-based mode!")
    ...
    def _load_model(self, model_path):
        ...
        # Initialize model architecture (using same config as training)
    ...
    def augment_pattern(self, synth_pattern, num_samples=5, temp_range=(0.1, 2.0),
                       ...
                       base_seed=None, return_metadata=False):
        """
    ...
    def augment_batch(self, synth_patterns, samples_per_pattern=5,
        ...
        Args:
            ...
            temp_range: Temperature conditioning range
            temp_mode: Temperature sampling mode
            noise_timestep_range: Range of diffusion timesteps
    ...
    def visualize_augmentation(self, original_pattern, augmented_patterns,
        ...
        if metadata is not None:
            ...
            scatter = axes[1,1].scatter(temps, timesteps, c=noise_levels, cmap='viridis', s=100, alpha=0.7)
...
def main():
    ...
    parser = argparse.ArgumentParser(description='XRD Pattern Augmenter (Refactored)')

    # Input/Output
    parser.add_argument('--input_file', default='data/xrd_dataset_labeled_dtw_window.pt', help='Input synthetic patterns file')
    ...
    print(f"🔬 XRD Pattern Augmenter - Model-Based (Refactored)")
    print("=" * 50)

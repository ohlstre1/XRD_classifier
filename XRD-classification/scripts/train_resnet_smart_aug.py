#!/usr/bin/env python3
"""
ResNet Classifier with Smart Classical Augmentation
====================================================

Key differences from train_resnet_classical_aug.py:
1. Lower augmentation intensity — preserves XRD peak positions/intensities
2. Class-aware augmentation — only augments underrepresented classes (< threshold samples)

Training strategy:
- Pre-generate augmented variations ONLY for classes with fewer than --aug_threshold samples
- Train on augmented synthetic + TRAIN portion of measured data (with light noise)
- Validate on measured data only (val split)
- Test on measured data only (test split)

Smart augmentation config (lower intensity):
- Peak shifting: prob=0.5, max_shift=3 (~1.2% strain, was 10)
- Peak broadening: prob=0.5, intensity_range=[0.05, 0.25] (was 0.8, [0.1, 0.8])
- Peak intensity variation: prob=0.5, range [0.9, 1.1] (+-10% texture, was [0.5, 1.5])
- Peak removal: disabled
- Background noise: prob=0.6, range [0.003, 0.01] (was [0.005, 0.03])

Dataset: data/xrd_dataset_labeled_dtw_window.pt (13,325 paired samples)

Data split (aug_threshold=10, num_variations=3):
    synth_xrd [13,325] ── smart aug (underrepresented only) ── [~20K-30K] ─┐
                                                                              ├──> TRAINING
...
def generate_smart_augmented(synth_patterns, synth_labels, augmenter,
                             num_variations=3, aug_threshold=10):
    """
    Pre-generate classical-augmented patterns only for underrepresented classes.

    Classes with fewer than aug_threshold samples get num_variations augmented
    copies per pattern. Classes at or above the threshold keep only their
    original synthetic patterns (no augmentation).
    ...
    """
    print(f"\n{'=' * 60}")
    print("Smart class-aware augmentation")
    print(f"{'=' * 60}")
    ...
    # Count how many patterns will be augmented
    patterns_to_aug = sum(1 for lbl in synth_labels.tolist() if lbl in classes_to_augment)
    patterns_skipped = len(synth_labels) - patterns_to_aug
    expected_new = patterns_to_aug * num_variations

    print(f"\n  Patterns to augment: {patterns_to_aug}")
    ...
    # Generate augmented copies only for underrepresented classes
    aug_patterns_list = []
    aug_labels_list = []

    for i in tqdm(range(len(synth_patterns)), desc="Smart augmentation"):
        label = synth_labels[i].item()
        ...
        aug_patterns_list.append(augmented)
        aug_labels_list.append(torch.full((num_variations,), label, dtype=torch.long))

    if aug_patterns_list:
    ...
    print(f"\n  Generated {len(combined_patterns) - len(synth_patterns)} augmented patterns")
...
def main():
    parser = argparse.ArgumentParser(
        description='Train ResNet Classifier with Smart Class-Aware Augmentation'
    )
    parser.add_argument('--data_path', type=str,
                       default='../../data/xrd_dataset_labeled_dtw_window.pt',
                       help='Path to dataset')
    parser.add_argument('--num_variations', type=int, default=3,
                       help='Number of augmented variations per pattern (for underrepresented classes)')
    ...
    parser.add_argument('--save_dir', type=str, default='./models/smart_aug_classifier',
    ...
    # Initialize classical augmenter with LOWER intensity config
    aug_config = {
        'peak_shifting': {
            'enabled': True,
            'probability': 0.5,
            'max_shift': 3  # ~1.2% strain (was 10)
        },
        'peak_broadening': {
            'enabled': True,
            'probability': 0.5,  # was 0.8
            'intensity_range': [0.05, 0.25],  # was [0.1, 0.8]
            'wavelength': 1.54056,
        ...
        'peak_variations': {
            'enabled': True,
            'probability': 0.5,  # was 0.7
            'variation_range': [0.9, 1.1],  # +-10% texture (was [0.5, 1.5])
            'threshold': 0.01
        },
        'peak_removal': {
        ...
        'background_noise': {
            'enabled': True,
            'probability': 0.6,
            'noise_level_range': [0.003, 0.01]  # was [0.005, 0.03]
        }
    }

    augmenter = ClassicalXRDAugmenter(config=aug_config, device='cpu', verbose=True)

    # Pre-generate smart class-aware augmented patterns
    aug_patterns, aug_labels = generate_smart_augmented(
        synth_patterns, synth_labels, augmenter,
    ...
    print(f"  Smart-augmented synthetic: {len(aug_patterns)} samples "
    ...
    if start_epoch > 0:
        ...
        print("Starting training with smart class-aware augmentation...")
    print("=" * 60)
    ...
    results = {
        'config': vars(args),
        'augmentation': 'smart_class_aware',
        'aug_strategy': {
            'aug_threshold': args.aug_threshold,
            'num_variations': args.num_variations,
        ...
        'aug_config': {
            'peak_shifting_max': 3,
            'broadening_prob': 0.5,
            'broadening_intensity': [0.05, 0.25],
            'variation_range': [0.9, 1.1],
            'variation_prob': 0.5,
            'noise_range': [0.003, 0.01],
        },

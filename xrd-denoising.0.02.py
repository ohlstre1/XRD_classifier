...
class DiffusionProcess:
    def __init__(self, num_timesteps=1000, schedule_type='cosine', beta_start=1e-4, beta_end=0.02, device='cpu'):
        ...
        self.device = device
        self.xrd_wavelength = 1.54  # Copper K-alpha in Angstroms (typical XRD wavelength)
    ...
    def estimate_noise_level(self, xrd_pattern, base_level=0.1):
        """
        ...
        background = torch.quantile(xrd_pattern, 0.1)
...
def denoise_xrd_dataset(model_path, dataset_path, output_dir, num_examples=10, device='cpu'):
    ...
    The script applies Scherrer equation-inspired peak broadening during the augmentation process.
    Peak broadening increases progressively with diffusion timestep, simulating how crystallite 
    size affects XRD peak width.
    """
...
if __name__ == "__main__":
    ...
    num_examples = 8  # Number of examples to process
    
    # Use CUDA if available
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    print("XRD Denoising Script with Scherrer-based Peak Broadening")
    print("======================================================")
    print("This script incorporates principles from Scherrer's equation,")
    print("which relates crystallite size to XRD peak broadening:")
    print("β = K·λ/(D·cos(θ))")
    print("- β: peak broadening")
    print("- K: shape factor (~0.9)")
    print("- λ: X-ray wavelength (default: 1.54Å Cu-Kα)")
    print("- D: crystallite size (smaller = broader peaks)")
    print("- θ: Bragg angle")
    print("\nBroadening increases progressively with diffusion timestep,")
    print("simulating the effects of decreasing crystallite size")
    print("======================================================\n")
